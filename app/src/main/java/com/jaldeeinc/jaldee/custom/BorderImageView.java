...
public class BorderImageView extends AppCompatImageView {
    ...
    public BorderImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        int index = a.getInt(R.styleable.BorderImageView_android_scaleType, -1);
        if (index >= 0) {
            this.setScaleType(sScaleTypeArray[index]);
        }

        this.mLeftTopCornerRadius = (float) a.getDimensionPixelSize(R.styleable.BorderImageView_left_top_corner_radius, 0);
        this.mRightTopCornerRadius = (float) a.getDimensionPixelSize(R.styleable.BorderImageView_right_top_corner_radius, 0);
        this.mLeftBottomCornerRadius = (float) a.getDimensionPixelSize(R.styleable.BorderImageView_left_bottom_corner_radius, 0);
        this.mRightBottomCornerRadius = (float) a.getDimensionPixelSize(R.styleable.BorderImageView_right_bottom_corner_radius, 0);
        if (this.mLeftTopCornerRadius >= 0.0F && this.mRightTopCornerRadius >= 0.0F && this.mLeftBottomCornerRadius >= 0.0F && this.mRightBottomCornerRadius >= 0.0F) {
            ...
            this.mBorderWidth = (float) a.getDimensionPixelSize(R.styleable.BorderImageView_border_width, 0);
            if (this.mBorderWidth < 0.0F) {
                throw new IllegalArgumentException("border width cannot be negative.");
            } else {
                this.mBorderColor = a.getColorStateList(R.styleable.BorderImageView_border_color);
                if (this.mBorderColor == null) {
                    this.mBorderColor = ColorStateList.valueOf(-16777216);
                }

                this.isOval = a.getBoolean(R.styleable.BorderImageView_oval, false);
                a.recycle();
                this.updateDrawable();
            }
        } else {
