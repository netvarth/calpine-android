...
public class LocationSearchAdapterNew extends RecyclerView.Adapter<LocationSearchAdapterNew.MyViewHolder> implements Filterable {
    ...
    Context mContext;
    LocationSearchAdapterNew.ValueFilter valueFilter;
    ArrayList<String> items;
    ArrayList<String> filteredItems;

    LocationSearchCallback callback;
    public LocationSearchAdapterNew(Context context, ArrayList<String> item) {
        mContext = context;
        items = item;
        filteredItems = item;
    ...
    @Override
    public LocationSearchAdapterNew.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.searchlocation_row, parent, false);

        return new LocationSearchAdapterNew.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(LocationSearchAdapterNew.MyViewHolder myViewHolder, final int position) {
        String searchdetailList = items.get(position);
        myViewHolder.tv_loc.setText(searchdetailList);
...
//        });
    ...
    public Filter getFilter() {
        if (valueFilter == null) {
            valueFilter = new LocationSearchAdapterNew.ValueFilter();
        }
        return valueFilter;
    }
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                for (int i = 0; i < filteredItems.size(); i++) {
                  ...
                  /*  if ((String.valueOf(filteredItems.get(i).getName()).toLowerCase())
                    ...
                    if ((String.valueOf(filteredItems.get(i).toLowerCase()).contains(constraint.toString().toLowerCase()))) {


                        LocationResponse loc=new LocationResponse();
                        loc.setName(filteredItems.get(i));
                        filterList.add(loc);
