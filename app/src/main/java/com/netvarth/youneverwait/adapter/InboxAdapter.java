...
import com.netvarth.youneverwait.activities.DetailInboxList;
        ...
        myViewHolder.linear_inbox_layout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for(int i=0;i<mInboxCompleteList.size();i++){
                    String mChkKey= mInboxCompleteList.get(i).getId() + "_" + mInboxCompleteList.get(i).getUserName();
                    if(mKey.equalsIgnoreCase(mChkKey)) {
                        InboxModel inbox = new InboxModel();
                        inbox.setKey(mChkKey);
                        inbox.setMsg(inboxList.getMsg());
                        inbox.setUserName(inboxList.getUserName());
                        inbox.setTimeStamp(inboxList.getTimeStamp());
